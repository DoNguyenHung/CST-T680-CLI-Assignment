...
func processCmdLineFlags() (AppOptType, error) {
	...
	// Pointer: listFlag, Name of flag: l, Default value: false, Documentation: "List all the item"
	...
	// The flag.Parse function on the next line uses this pointer to set the bool variable based on
	// the flags passed in by the user. We are then able to check the value of this bool pointer by dereferencing the pointer.
	flag.Parse()

	var appOpt AppOptType = INVALID_APP_OPT

	//show help if no flags are set
	// Prints usage (descripton of each flag)
	if len(os.Args) == 1 {
		flag.Usage()
	...
	// accordingly
	// Checks through all the flags to see if anyone is entered.
